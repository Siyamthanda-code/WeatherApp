document.addEventListener('DOMContentLoaded', function() {
    ...
    const urlByCoords = (lat, lon) => `https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&appid=${apiKey}&units=metric`;
    ...
    function createSnowflakes() {
        ...
        if (!snowContainer) return; // Safety check
        
        snowContainer.innerHTML = '';
        
        for (let i = 0; i < 30; i++) {
            const flake = document.createElement('i');
            flake.className = 'fas fa-snowflake';
    ...
    function updateWeatherAnimation(weatherCode) {
        ...
        if (thunderstorm) thunderstorm.style.display = 'none';
        if (partiallyCloudy) partiallyCloudy.style.display = 'none';
        if (snow) snow.style.display = 'none';

        // Show appropriate animation based on weather code
        if (weatherCode === 800) {
            if (sun) sun.style.display = 'block'; // Clear sky
            if (clear) clear.style.display = 'block';
        } else if (weatherCode >= 801 && weatherCode <= 802) {
            if (partiallyCloudy) partiallyCloudy.style.display = 'block'; // Partially cloudy
        } else if (weatherCode >= 803 && weatherCode <= 804) {
            if (clouds) clouds.style.display = 'flex'; // Mostly cloudy
        } else if (weatherCode >= 200 && weatherCode < 300) {
            if (thunderstorm) thunderstorm.style.display = 'block'; // Thunderstorm
